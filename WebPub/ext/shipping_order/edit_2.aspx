<%@ Page Title="" Language="C#" MasterPageFile="~/master_page/client.master" AutoEventWireup="true" CodeFile="edit_2.aspx.cs" Inherits="ext_shipping_order_edit_2" %>

<%@ Reference Control="~/include/client/ext/shipping_order/header_discount_block.ascx" %>
<%@ Register Src="~/include/client/ext/shipping_order/edit_2/goods_block.ascx" TagPrefix="UC" TagName="GoodsList" %>
        ...
        <asp:PlaceHolder ID="phCreateOrderOper" runat="server">
            <div class="rondane_box">
                <div class="btnBox">
                    <div>
                        <div>
                            <asp:LinkButton ID="btnCreateGoods" Text="建立出貨品項" runat="server" OnClick="btnCreateGoods_Click" />
                        </div>
            ...
            <div class="rondane_box">
                ...
                <asp:UpdatePanel UpdateMode="Conditional" runat="server">
                    <ContentTemplate>
                        ...
                        <UC:GoodsList ID="ucGoodsList" runat="server" />
                    </ContentTemplate>
                    <%--<Triggers>
        ...
        var orderItemHelper = (function () {
            ...
            return {
                openWindow: function (cntrSId) {
                    ...
                    <%--外銷訂單品項--%>
                    window.open('<%=Seec.Marketing.SystemDefine.WebSiteRoot %>popup/ext/shipping_order/det_goods.aspx?cntrSId='.concat(cntrSId).concat('&customerId=').concat(customerId), 'goods', 'width=900, height=700, top=100, left=100, scrollbars=1');
                },
                ...
                checkMaxQty: function (cntrSId) {
                    var container = $(".dev-goods-det-item-".concat(cntrSId));
                    if (!container) {
                        return;
                    }
                ...
                calcGoodsItemAmt: function (container, headerDcts, recalcUnitPriceDct) {
                    ...
                    var subtotalQty = 0;
                    $(".dev-goods-det-item", container).each(function () {
                        $(".dev-goods-qty", $(this)).each(function () {
                            subtotalQty += parseInt($(this).val(), 10);
                        });
                    });

                    if (isNaN(subtotalQty)) {
                        $(".dev-goods-subtotal-qty-disp", container).html("");
                        $(".dev-goods-subtotal-qty :input[type=hidden]", container).val("")
                    } else {
                        goodsInfo.qty = subtotalQty;
                        $(".dev-goods-subtotal-qty-disp", container).html(accounting.formatNumber(goodsInfo.qty, 0));
                        $(".dev-goods-subtotal-qty :input[type=hidden]", container).val(goodsInfo.qty)
                    }
