...
public partial class ext_shipping_order_edit_2 : System.Web.UI.Page
{
    #region 網頁屬性
        ...
        try
        {
            ...
            if (this._extShippingOrderSId != null)
            {
                ...
                if (this.SetEditData(this._extShippingOrderSId))
                {
                    ...
                    if (!this.IsPostBack)
                    {
                        if (this.OrigInfo.Info.DetLayout == 1)
                        {
            ...
            else
            {
                ...
                if (!this.IsPostBack)
                {
                    this.InputInfo = Session[SessionDefine.SystemBuffer] as ExtShippingOrderHelper.InputInfoSet;
                    if (this.InputInfo != null)
                    {
                        #region 從來源資料直接初始至開始建立出貨單品項
                        //產生一組新的系統代號並暫存
                        this.hidSpecSId.Value = this.InputInfo.Info.SId.Value;

                        this.hidCustomerId.Value = ConvertLib.ToStr(this.InputInfo.Info.CustomerId, string.Empty);

                        this.litCdt.Text = ConvertLib.ToStr(this.InputInfo.Info.Cdt, string.Empty, "yyyy-MM-dd");
                        this.txtCdd.Text = ConvertLib.ToStr(this.InputInfo.Info.Cdd, string.Empty, "yyyy-MM-dd");
                        this.txtEdd.Text = ConvertLib.ToStr(this.InputInfo.Info.Edd, string.Empty, "yyyy-MM-dd");
                        this.txtEta.Text = ConvertLib.ToStr(this.InputInfo.Info.Eta, string.Empty, "yyyy-MM-dd");

                        this.lblCurrencyCode.Text = this.InputInfo.Info.CurrencyCode;
                        this.hidCustomerName.Value = this.InputInfo.Info.CustomerName;
                        this.hidPriceListId.Value = ConvertLib.ToStr(this.InputInfo.Info.PriceListId, string.Empty);
        ...
        try
        {
            ...
            if (this.OrigInfo != null)
            {
                ...
                if (this.OrigInfo.DetInfos != null && this.OrigInfo.DetInfos.Count > 0)
                {
                    ...
                    var partNos = this.OrigInfo.DetInfos.GroupBy(q => q.PartNo).Select(q => q.Key).OrderBy(q => q).ToArray();
                    for (int i = 0; i < partNos.Length; i++)
                    {
                        var partNo = partNos[i];

                        //排序訂單編號, 最新的在前面, 共用資訊以最新的為主.
                        var detInfos = this.OrigInfo.DetInfos.Where(q => q.PartNo.Equals(partNo)).OrderByDescending(q => q.ExtQuotnOdrNo).ToArray();
                        if (detInfos.Length > 0)
                        {
                            var itemEditInfo = new ExtShippingOrderHelper.GoodsItemEditInfo_PNBase()
                            {
                                SeqNo = i + 1,
                                Model = detInfos[0].Model,
                                PartNo = detInfos[0].PartNo,
                                UnitPrice = detInfos[0].UnitPrice,
                                UnitPriceDct = detInfos[0].UnitPriceDct,
                                Discount = detInfos[0].Discount,
                                PaidAmt = detInfos.Sum(q => q.PaidAmt),
                                Rmk = detInfos[0].Rmk
                            };
                            editInfo.Items.Add(itemEditInfo);

                            //不顯示在手量
                            //itemEditInfo.ErpOnHand = ErpHelper.GetOnHandQty(onHandInfos, whse, detInfos[0].Item);
                            itemEditInfo.ListPrice = ErpHelper.GetPriceBookListPrice(priceBookInfos, detInfos[0].PartNo);
                            ...
                            foreach (var detInfo in detInfos)
                            {
                                var editDetInfo = new ExtShippingOrderHelper.GoodsItemDetEditInfo_PNBase()
                                {
                                    ...
                                    OdrNo = detInfo.ExtQuotnOdrNo,
                                    Qty = detInfo.Qty
                                };
                                itemEditInfo.DetItems.Add(editDetInfo);
                            }
                        }
                    }

                    this.ucGoodsList.SetInfo(editInfo);
                    #endregion
                    #endregion
                }
                #endregion
    ...
    ExtShippingOrderHelper.InputInfoSet BindingInputted(bool bindingAll)
    {
        ...
        try
        {
            ...
            //明細品項的加入方式
            this.InputInfo.Info.DetLayout = 2;
            ...
            if (bindingAll)
            {
                ...
                #region 出貨單品項
                var editInfo = this.ucGoodsList.GetInfo();
                this.InputInfo.GoodsEditInfos_PNBase.Add(editInfo);

                foreach (var item in editInfo.Items)
                {
                    foreach (var detItem in item.DetItems)
                    {
                        var detInfo = new ExtShippingOrderDet.InputInfo()
                        {
                            SId = detItem.SId,
                            ExtShippingOrderSId = this.InputInfo.Info.SId,
                            ExtOrderDetSId = detItem.ExtOrderDetSId,
                            Model = item.Model,
                            PartNo = item.PartNo,
                            Qty = detItem.Qty,
    ...
    bool CheckInputted(CheckMode checkMode)
    {
        ...
        switch (checkMode)
        {
            ...
            case CheckMode.Modify:
                ...
                if (this.InputInfo.GoodsEditInfos_PNBase.Count == 0)
                {
                    //理論上不會進來這邊, 保險起見.
                ...
                else
                {
                    #region 品項驗證
                    var totalGoodsCnt = 0;
                    for (int i = 0; i < this.InputInfo.GoodsEditInfos_PNBase.Count; i++)
                    {
                        totalGoodsCnt += this.InputInfo.GoodsEditInfos_PNBase[i].Items.Sum(q => q.DetItems.Count);
                    }
                    ...
                    else
                    {
                        for (int i = 0; i < this.InputInfo.GoodsEditInfos_PNBase.Count; i++)
                        {
                            var editInfo = this.InputInfo.GoodsEditInfos_PNBase[i];
                            ...
                            foreach (var itemInfo in editInfo.Items)
                            {
                                ...
                                {
                                    //理論上不會進來這邊, 保險起見.
                                    errMsgs.Add(string.Format("序號 {0} - 未包含料號", itemInfo.SeqNo));
                                }

                                foreach (var detItemInfo in itemInfo.DetItems)
                                {
                                    if (detItemInfo.Qty == null || detItemInfo.Qty < 1)
                                    {
                                        errMsgs.Add(string.Format("序號 {0} 訂單編號 {1} - 請輸入「數量」 (或不得為 0)", itemInfo.SeqNo, detItemInfo.OdrNo));
                                    }
                                    else
                                    {
                                        if (detItemInfo.Qty > detItemInfo.MaxQty)
                                        {
                                            errMsgs.Add(string.Format("序號 {0} 訂單編號 {1} - 「數量」超過限定的最大值", itemInfo.SeqNo, detItemInfo.OdrNo));
                                        }
                                    }
                                }

                                //if (itemInfo.DetItems.Sum(q => q.Qty) > ConvertLib.ToInt(itemInfo.ErpOnHand, 0))
                                //{
                                //    errMsgs.Add(string.Format("序號 {0} - 「數量小計」超過庫存量的最大值", itemInfo.SeqNo));
                                //}

                                if (itemInfo.ListPrice == null)
                                {
                                    errMsgs.Add(string.Format("序號 {0} - 找不到品項「牌價」", itemInfo.SeqNo));
                                }

                                if (itemInfo.UnitPrice == null)
                                {
                                    errMsgs.Add(string.Format("序號 {0} - 請輸入「單價」", itemInfo.SeqNo));
                                }

                                if (itemInfo.UnitPriceDct == null)
                                {
                                    errMsgs.Add(string.Format("序號 {0} - 請輸入「折扣後單價」", itemInfo.SeqNo));
                                }
                            }
                        }
                    }
