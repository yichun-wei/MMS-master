...
public partial class ext_quotn_index : System.Web.UI.Page
{
    #region 網頁屬性
    ...
    private bool InitPage()
    {
        ...
        breadcrumbs.Add(string.Format("<a href='{0}'>外銷報價單</a>", QueryStringParsing.CurrentRelativeUri.OriginalString));
        ...
        try
        {
            ...
            {
                //狀態
                this.lstStatusList.Items.AddRange(ExtQuotnHelper.GetExtQuotnStatusItems(true));
                ...
                returner = entityExtQuotn.GetCreaterInfo(Int32.MaxValue, new SqlOrder("ACCT", Sort.Ascending), IncludeScope.All, new string[] { "SID", "ACCT", "NAME" });
                if (returner.IsCompletedAndContinue)
                {
                    ...
                    {
                        this.lstCreaterList.Items.Add(new ListItem(info.Name, info.SId.Value));
                    }
                }
            }
        }
    ...
    class SearchConds
    {
        ...
        public ExtQuotn.InfoViewConds Conds { get; set; }
    ...
    SearchConds GetSearchConds()
    {
        ...
        if (string.IsNullOrWhiteSpace(this.hidSearchConds.Value))
        {
            ...
            conds.Conds = new ExtQuotn.InfoViewConds
                (
                   DefVal.SIds,
                   ConvertLib.ToSIds(this.lstCreaterList.SelectedValue),
                   statuses,
                   DefVal.DT,
                   DefVal.DT,
        ...
        else
        {
            ...
            #region 還原查詢條件
            if (conds.Conds.CSIds != null && conds.Conds.CSIds.Length > 0)
            {
                WebUtilBox.SetListControlSelected(conds.Conds.CSIds[0].Value, this.lstCreaterList);
            }
            else
            {
                WebUtilBox.SetListControlSelected(string.Empty, this.lstCreaterList);
            }
    ...
    void PageList()
    {
        ...
        try
        {
            ...
            returner = entityExtQuotn.GetInfoViewByCompoundSearchCount(conds.Conds, conds.KeywordCols.ToArray(), conds.Keyword, false, dataScope);
            ...
            returner = entityExtQuotn.GetInfoViewByCompoundSearch(conds.Conds, conds.KeywordCols.ToArray(), conds.Keyword, false, flipper, sorting, dataScope);
            ...
            if (returner.IsCompletedAndContinue)
            {
                var infos = ExtQuotn.InfoView.Binding(returner.DataSet.Tables[0]);
                ...
                for (int i = 0; i < infos.Length; i++)
                {
                    ...
                    htmlTd = new TableCell();
                    htmlTd.Text = ExtQuotnHelper.GetExtQuotnStatusName(info.Status, info.IsReadjust, info.IsCancel);
                    htmlTr.Cells.Add(htmlTd);
