...
public partial class include_client_ext_order_edit_goods_block : System.Web.UI.UserControl
{
    const string USER_CONTROL_LOAD_SEQ = "UserControlLoadSeq";
    List<ASP.include_client_ext_order_edit_goods_item_block_ascx> _blockGoodsItemList = new List<ASP.include_client_ext_order_edit_goods_item_block_ascx>();
        ...
        if (this.IsPostBack)
        {
            if (ViewState[USER_CONTROL_LOAD_SEQ] != null)
            {
                ...
                for (int i = 0; i < loadSeq.Length; i++)
                {
                    ...
                    {
                        case "GoodsItem":
                            var blockGoodsItem = (ASP.include_client_ext_order_edit_goods_item_block_ascx)this.LoadControl("~/include/client/ext/order/edit/goods_item_block.ascx");
    ...
    public void SetInfo(ExtOrderHelper.GoodsEditInfo editInfo)
    {
        ...
        if (editInfo.Items != null && editInfo.Items.Count > 0)
        {
            ...
            {
                var block = (ASP.include_client_ext_order_edit_goods_item_block_ascx)this.LoadControl("~/include/client/ext/order/edit/goods_item_block.ascx");
