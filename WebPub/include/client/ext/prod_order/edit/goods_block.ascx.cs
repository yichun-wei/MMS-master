...
public partial class include_client_ext_prod_order_edit_goods_block : System.Web.UI.UserControl
{
    const string USER_CONTROL_LOAD_SEQ = "UserControlLoadSeq";
    List<ASP.include_client_ext_prod_order_edit_goods_item_block_ascx> _blockGoodsItemList = new List<ASP.include_client_ext_prod_order_edit_goods_item_block_ascx>();
        ...
        if (this.IsPostBack)
        {
            if (ViewState[USER_CONTROL_LOAD_SEQ] != null)
            {
                ...
                for (int i = 0; i < loadSeq.Length; i++)
                {
                    ...
                    {
                        case "GoodsItem":
                            var blockGoodsItem = (ASP.include_client_ext_prod_order_edit_goods_item_block_ascx)this.LoadControl("~/include/client/ext/prod_order/edit/goods_item_block.ascx");
    ...
    #region 設定資訊
    public void SetInfo(ExtProdOrderHelper.GoodsEditInfo editInfo)
    {
        if (editInfo == null)
        {
            this.Visible = false;
        ...
        if (editInfo.Items != null && editInfo.Items.Count > 0)
        {
            ...
            {
                var block = (ASP.include_client_ext_prod_order_edit_goods_item_block_ascx)this.LoadControl("~/include/client/ext/prod_order/edit/goods_item_block.ascx");
    ...
    #region 取得輸入資訊
    public ExtProdOrderHelper.GoodsEditInfo GetInfo()
    {
