...
public partial class include_client_ext_quotn_edit_manual_goods_block : System.Web.UI.UserControl
{
    const string USER_CONTROL_LOAD_SEQ = "UserControlLoadSeq";
    List<ASP.include_client_ext_quotn_edit_manual_goods_item_block_ascx> _blockGoodsItemList = new List<ASP.include_client_ext_quotn_edit_manual_goods_item_block_ascx>();
        ...
        if (this.IsPostBack)
        {
            if (ViewState[USER_CONTROL_LOAD_SEQ] != null)
            {
                ...
                for (int i = 0; i < loadSeq.Length; i++)
                {
                    ...
                    {
                        case "GoodsItem":
                            var blockGoodsItem = (ASP.include_client_ext_quotn_edit_manual_goods_item_block_ascx)this.LoadControl("~/include/client/ext/quotn/edit/manual/goods_item_block.ascx");
                            blockGoodsItem.Remove += new include_client_ext_quotn_edit_manual_goods_item_block.RemoveEventHandler(GoodsItem_OnRemove);
    ...
    #region 設定資訊
    public void SetInfo(ExtQuotnHelper.GoodsEditInfo editInfo)
    {
        if (editInfo == null)
        {
            this.Visible = false;
        ...
        if (editInfo.Items != null && editInfo.Items.Count > 0)
        {
            ...
            {
                var block = (ASP.include_client_ext_quotn_edit_manual_goods_item_block_ascx)this.LoadControl("~/include/client/ext/quotn/edit/manual/goods_item_block.ascx");
                block.Remove += new include_client_ext_quotn_edit_manual_goods_item_block.RemoveEventHandler(GoodsItem_OnRemove);
    ...
    #region 取得輸入資訊
    public ExtQuotnHelper.GoodsEditInfo GetInfo()
    {
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        //var editInfo = new ExtQuotnHelper.GoodsItemEditInfo()
        //{
        ...
        this.Add(new ExtQuotnHelper.GoodsItemEditInfo[] { new ExtQuotnHelper.GoodsItemEditInfo() });

        //計算整張訂單金額
        WebUtilBox.RegisterScript(this.Page, "orderItemHelper.calcOrderAmt();");
    }

    public void Add(ExtQuotnHelper.GoodsItemEditInfo[] editInfos)
    {
