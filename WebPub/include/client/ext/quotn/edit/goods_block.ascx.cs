...
public partial class include_client_ext_quotn_edit_goods_block : System.Web.UI.UserControl
{
    const string USER_CONTROL_LOAD_SEQ = "UserControlLoadSeq";
    List<ASP.include_client_ext_quotn_edit_goods_item_block_ascx> _blockGoodsItemList = new List<ASP.include_client_ext_quotn_edit_goods_item_block_ascx>();
        ...
        if (this.IsPostBack)
        {
            if (ViewState[USER_CONTROL_LOAD_SEQ] != null)
            {
                ...
                for (int i = 0; i < loadSeq.Length; i++)
                {
                    ...
                    {
                        case "GoodsItem":
                            var blockGoodsItem = (ASP.include_client_ext_quotn_edit_goods_item_block_ascx)this.LoadControl("~/include/client/ext/quotn/edit/goods_item_block.ascx");
                            blockGoodsItem.Remove += new include_client_ext_quotn_edit_goods_item_block.RemoveEventHandler(GoodsItem_OnRemove);
    ...
    public void SetInfo(ExtQuotnHelper.GoodsEditInfo editInfo)
    {
        ...
        if (editInfo.Items != null && editInfo.Items.Count > 0)
        {
            ...
            {
                var block = (ASP.include_client_ext_quotn_edit_goods_item_block_ascx)this.LoadControl("~/include/client/ext/quotn/edit/goods_item_block.ascx");
                block.Remove += new include_client_ext_quotn_edit_goods_item_block.RemoveEventHandler(GoodsItem_OnRemove);
    ...
    protected void GoodsItem_OnRemove(object sender, EventArgs e)
    {
        ...
        var itemEditInfo = ((ASP.include_client_ext_quotn_edit_goods_item_block_ascx)sender).GetInfo();
        //一般品項
        //existedGoodsItems.Remove(itemEditInfo.PartNo);
        //[by fan] 加入型號「型號[#]料號」
        existedGoodsItems.Remove(string.Format("{1}{0}{2}", SystemDefine.JoinSeparator, ConvertLib.ToBase64Encoding(itemEditInfo.Model), itemEditInfo.PartNo));
