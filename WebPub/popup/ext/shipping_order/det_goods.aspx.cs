...
public partial class popup_ext_shipping_order_det_goods : System.Web.UI.Page
{
    #region 網頁屬性
    /// <summary>
    /// 主版。
    ...
    private bool InitVital()
    {
        ...
        if (this.ContainerSId == null || this.CustomerId == null)
        {
    ...
    class SearchConds
    {
        ...
        public ExtOrderDet.CrossOrderItemsGrpInfoViewConds Conds { get; set; }
    ...
    SearchConds GetSearchConds()
    {
        ...
        if (string.IsNullOrWhiteSpace(this.hidSearchConds.Value))
        {
            ...
            conds.KeywordCols.Add("MODEL");
            conds.KeywordCols.Add("PART_NO");

            conds.Conds = new ExtOrderDet.CrossOrderItemsGrpInfoViewConds
                (
                   DefVal.SIds,
                   this.CustomerId,
                   DefVal.SIds,
        ...
        else
        {
            ...
            bool hasChgCatConds =
                conds.Conds.ExportItemType != this.lstItemType.SelectedValue
                || conds.Conds.Category1 != this.lstCat_1.SelectedValue
                || conds.Conds.Category2 != this.lstCat_2.SelectedValue
                || conds.Conds.Category3 != this.lstCat_3.SelectedValue
                || conds.Conds.Category4 != this.lstCat_4.SelectedValue
                || conds.Conds.Category5 != this.lstCat_5.SelectedValue;

            if (hasChgCatConds)
            {
                var extItemCatConds = ExtItemHelper.GetExtItemCatConds(true);
                IExtItemCats[] filterCats = extItemCatConds.ExtItemCats.Where(q => conds.Conds.ExportItemType.Equals(q.ExportItemType)).ToArray();

                #region 產品別
                if (string.IsNullOrWhiteSpace(conds.Conds.ExportItemType))
                {
                    //之後的連續清空
                ...
                {
                    if (conds.Conds.ExportItemType != this.lstItemType.SelectedValue)
                    {
                        #region 重置分類
                        this.lstCat_1.Items.Clear();
                        this.lstCat_2.Items.Clear();
                        ...
                        var extItemTypeInfo = extItemCatConds.ExtItemTypes.Where(q => conds.Conds.ExportItemType.Equals(q.ExportItemType)).Single();
                    ...
                    if (!string.IsNullOrWhiteSpace(WebUtilBox.SetListControlSelected(conds.Conds.ExportItemType, this.lstItemType)))
                    {
                        //若無下階值, 則下下階之後的會直接連續清空.
                        filterCats = this.ResetCat_1_List(filterCats, conds.Conds.Category1);
                    }
                    else
                    {
                ...
                {
                    if (string.IsNullOrWhiteSpace(conds.Conds.Category1))
                    {
                        //之後的連續清空
                    ...
                    {
                        if (!string.IsNullOrWhiteSpace(WebUtilBox.SetListControlSelected(conds.Conds.Category1, this.lstCat_1)))
                        {
                            //若無下階值, 則下下階之後的會直接連續清空.
                            filterCats = this.ResetCat_2_List(filterCats, conds.Conds.Category2);
                        }
                        else
                        {
                            //若指定後仍是找不到, 或下階為空值, 則之後的連續清空.
                ...
                if (filterCats != null && filterCats.Length > 0)
                {
                    if (string.IsNullOrWhiteSpace(conds.Conds.Category2))
                    {
                        //之後的連續清空
                    ...
                    {
                        if (!string.IsNullOrWhiteSpace(WebUtilBox.SetListControlSelected(conds.Conds.Category2, this.lstCat_2)))
                        {
                            //若無下階值, 則下下階之後的會直接連續清空.
                            filterCats = this.ResetCat_3_List(filterCats, conds.Conds.Category3);
                        }
                        else
                        {
                            //若指定後仍是找不到, 或下階為空值, 則之後的連續清空.
                ...
                if (filterCats != null && filterCats.Length > 0)
                {
                    if (string.IsNullOrWhiteSpace(conds.Conds.Category3))
                    {
                        //之後的連續清空
                    ...
                    {
                        if (!string.IsNullOrWhiteSpace(WebUtilBox.SetListControlSelected(conds.Conds.Category3, this.lstCat_3)))
                        {
                            //若無下階值, 則下下階之後的會直接連續清空.
                            filterCats = this.ResetCat_4_List(filterCats, conds.Conds.Category4);
                        }
                        else
                        {
                            //若指定後仍是找不到, 或下階為空值, 則之後的連續清空.
                ...
                if (filterCats != null && filterCats.Length > 0)
                {
                    if (string.IsNullOrWhiteSpace(conds.Conds.Category4))
                    {
                        //之後的連續清空
                    ...
                    {
                        if (!string.IsNullOrWhiteSpace(WebUtilBox.SetListControlSelected(conds.Conds.Category4, this.lstCat_4)))
                        {
                            //若無下階值, 則下下階之後的會直接連續清空.
                            filterCats = this.ResetCat_5_List(filterCats, conds.Conds.Category5);
                        }
                        else
                        {
                            //若指定後仍是找不到, 或下階為空值, 則之後的連續清空.
                ...
                if (filterCats != null && filterCats.Length > 0)
                {
                    if (string.IsNullOrWhiteSpace(conds.Conds.Category5))
                    {
                        WebUtilBox.SetListControlSelected(string.Empty, this.lstCat_5);
                    }
                    else
                    {
                        WebUtilBox.SetListControlSelected(conds.Conds.Category5, this.lstCat_5);
                    }
                }
                #endregion
            }
    ...
    void ExtOrderDetList()
    {
        ...
        try
        {
            ...
            //returner = entityExtOrderDet.GetCrossOrderItemsGrpInfoViewCount(conds.Conds);
            returner = entityExtOrderDet.GetCrossOrderItemsGrpInfoViewByCompoundSearchCount(conds.Conds, conds.KeywordCols.ToArray(), conds.Keyword, false);
            ...
            //returner = entityExtOrderDet.GetCrossOrderItemsGrpInfoView(conds.Conds, flipper, sorting);
            returner = entityExtOrderDet.GetCrossOrderItemsGrpInfoViewByCompoundSearch(conds.Conds, conds.KeywordCols.ToArray(), conds.Keyword, false, flipper, sorting);

            if (returner.IsCompletedAndContinue)
            {
                var infos = ExtOrderDet.CrossOrderItemsGrpInfoView.Binding(returner.DataSet.Tables[0]);
                ...
                for (int i = 0; i < infos.Length; i++)
                {
                    ...
                    if (this.IsSingleMode)
                    {
                        ...
                        htmlTd.Text = existedGoodsItems.Contains(info.PartNo) ? string.Empty : string.Format("<input type='button' value='選擇' class='dev-sel' partno='{0}' />", info.PartNo);
                    }
                    else
                    {
                        htmlTd.Text = existedGoodsItems.Contains(info.PartNo) ? string.Empty : string.Format("<input type='checkbox' class='dev-sel' value='{0}' />", info.PartNo);
                    }
                    htmlTr.Cells.Add(htmlTd);
                }
            }
