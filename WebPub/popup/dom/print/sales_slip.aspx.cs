...
public partial class popup_dom_print_sales_slip : System.Web.UI.Page
{
    #region 網頁屬性
    /// <summary>
    /// 網頁的控制操作。
    ...
    #region 列印銷貨清單
    #region 原寫法 (未分 Sheet) - 已註解
    //void Print()
    //{
    //    //網頁未完成初始時不執行
    //    if (!this.HasInitial) { return; }

    //    var fileName = string.Format("銷貨清單_{0}.xlsx", this.OrigInfo.Info.OdrNo);

    //    string templatePath = WebUtilBox.MapPath("~/sample/dom/銷貨清單.xlsx");
    ...
    //        row.GetCell(NpoiHelper.GetZeroBasedCellIdx("B")).SetCellValue(ConvertLib.TraditionalToSimplified(string.Join(" ", string.Format("廈門士林電機有限公司{0}分公司", this.OrigInfo.Info.DomDistName).ToArray()))); //表頭-內銷地區

    //        curRowIdx = 1;
    //        row = sheet.GetRow(curRowIdx);
    //        string domDistTel = string.Empty, domDistFax = string.Empty, domDistAddr = string.Empty;
    //        #region 自訂欄位
    ...
    //        foreach (var erpDctRel in this.OrigInfo.HeaderDiscountInfos)
    //        {
    //            var headerDct = erpDctRel.Discount;
    ...
    //                curCellIdx += 2; //重量
    //            }
    //            row.GetCell(++curCellIdx).SetCellValue(detInfo.Summary); //型號
        ...
        using (FileStream fs = new FileStream(templatePath, FileMode.Open, FileAccess.Read))
        {
           ...
           // row.GetCell(NpoiHelper.GetZeroBasedCellIdx("C")).SetCellValue(ConvertLib.TraditionalToSimplified(this.OrigInfo.Info.CustomerAddr)); //地址
            ...
            for (int pageNo = 1; pageNo <= totalSheets; pageNo++)
            {
                ...
                this.CreateSalesSlipSheet(sheet.CopySheet(string.Format("Page {0}", pageNo), true), detInfos, beginIdx, pageNo == totalSheets);
            }
            ...
            if (false)
            {
                ...
                for (int i = 0; i < this.OrigInfo.DetInfos.Count; i++)
                {
                    ...
                    //20171025 12842 Vicky 型號欄位內容改為 摘要 → XS型號(摘要)
    ...
    void CreateSalesSlipSheet(ISheet sheet, DomOrderDet.InfoView[] detInfos, int beginIdx, bool lastPage)
    {
        IRow row;
        int curRowIdx;
        ...
        for (int i = 0; i < detInfos.Length; i++)
        {
            ...
            #region 原本的單價和小計皆各自計算折扣後的金額 (已註解備份)
            ////單價要被扣掉的價錢
            //float unitPriceDownPrice = (float)MathLib.Round(detInfo.UnitPrice * totalHeaderDct, 4);
            ////折扣後的單價
            //float unitPriceDct = detInfo.UnitPrice - unitPriceDownPrice;
            ...
            //float subtotalDct = detInfo.PaidAmt - subtotalDownPrice;
            ////折扣後的含稅小計
            //float subtotalAmt = subtotalDct * (1 + DomOrderHelper.RmbTax);

            //row.GetCell(++curCellIdx).SetCellValue(MathLib.Round(subtotalAmt / detInfo.Qty, 4)); //單價
            //row.GetCell(++curCellIdx).SetCellValue(subtotalAmt); //含稅金額 
            #endregion

            //[20160224 by 米雪兒]
            #region 改為「單價，含稅小計，分別計算」2016.2.24
            //單價要被扣掉的價錢
            float unitPriceDownPrice = (float)MathLib.Round(detInfo.UnitPrice * totalHeaderDct, 4);
            //折扣後的單價
            float unitPriceDct = detInfo.UnitPrice - unitPriceDownPrice;
            //含稅單價 2016.2.24
            float unitPriceDctTax = (float)MathLib.Round(unitPriceDct * (1 + DomOrderHelper.RmbTax), 4);
            //折扣後的含稅小計 2016.2.24
            float subtotalAmt = unitPriceDctTax * detInfo.Qty; ;

            row.GetCell(++curCellIdx).SetCellValue(unitPriceDctTax); //單價
            row.GetCell(++curCellIdx).SetCellValue(subtotalAmt); //含稅金額 
            #endregion
        }
        ...
        if (lastPage)
        {
            ...
            //[20160224 by 米雪兒] 修正如果明細行只有一筆，唯一那一筆 與 總計 有的時候會差0.01(四捨五入的問題)
            //[20170717 by 儀淳] 修正若明細行剛好25筆，第25筆小計內容呈現訂單總金額
            if (beginIdx==1 && detInfos.Length == 1 && this.OrigInfo.Info.DctTotalAmt.Value != sheet.GetRow(14).GetCell(11).NumericCellValue)
            {
                sheet.GetRow(14).GetCell(11).SetCellValue(this.OrigInfo.Info.DctTotalAmt.Value); //含稅金額 
            }
        }
        else
        {
