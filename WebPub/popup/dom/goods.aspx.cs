...
public partial class popup_dom_goods : System.Web.UI.Page
{
    #region 網頁屬性
    /// <summary>
    /// 主版。
    ...
    private bool InitVital()
    {
        ...
        if (this.ContainerSId == null || this._customerId == null)
        {
    ...
    private bool InitPage()
    {
        ...
        try
        {
            if (!this.IsPostBack)
            {
                ...
                returner = entityPGOrderDet.GetGroupInfo(new PGOrderDet.InfoViewConds(DefVal.SIds, DefVal.SId, DefVal.Str, 1, DefVal.Str, DefVal.Str, DefVal.Str, this._customerId, false, true, IncludeScope.OnlyNotMarkDeleted), ConvertLib.ToStrs("PG_ORDER_ODR_NO"), Sort.Ascending);
                if (returner.IsCompletedAndContinue)
                {
                    var rows = returner.DataSet.Tables[0].Rows;
                    ...
                    foreach (DataRow row in rows)
                    {
                        this.lstSourceList.Items.Add(new ListItem(row["PG_ORDER_ODR_NO"].ToString(), row["PG_ORDER_ODR_NO"].ToString()));
                    }
                }
            }
        }
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                switch (this.lstSearchBy.SelectedValue)
                {
                    case "2":
                        this.PGOrderDetList();
                        break;
    ...
    {
        WebUtilBox.SetListControlSelected("1", this.lstSearchBy);
        this.Page_DoSearch(null, EventArgs.Empty);
    }

    protected void btnSearchByPGOrder_Click(object sender, EventArgs e)
    {
        WebUtilBox.SetListControlSelected("2", this.lstSearchBy);
        this.Page_DoSearch(null, EventArgs.Empty);
    }
    ...
    void PGOrderDetList()
    {
        ...
        try
        {
            ...
            sorting.Add("PART_NO", Sort.Ascending);
