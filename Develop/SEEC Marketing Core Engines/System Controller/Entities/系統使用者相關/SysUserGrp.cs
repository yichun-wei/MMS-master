...
namespace Seec.Marketing.SystemEngines
{
    public partial class DBTableDefine
    {
        ...
        /// 系統使用者群組。
        /// </summary>
        public const string SYS_USER_GRP = "SYS_USER_GRP";
    }

    /// <summary>
    /// 系統使用者群組的類別實作。
    /// </summary>
    public class SysUserGrp : SystemHandler
    {
        /// <summary>
        ...
        public SysUserGrp(string connInfo)
            : base(EntityHelper.GetEntity(connInfo, DBTableDefine.SYS_USER_GRP))
        {
        ...
        /// <param name="sysUserGrpSId">指定的系統使用者群組系統代號。</param>
        /// <param name="enabled">資料的啟用狀態。</param>
        /// <param name="name">名稱。</param>
        /// <param name="userRight">使用者權限（null 或 empty 將自動略過操作）。</param>
        ...
        public Returner Add(ISystemId actorSId, ISystemId sysUserGrpSId, bool enabled, string name, string userRight, string desc, int sort)
        {
            if (actorSId == null) { throw new ArgumentNullException("actorSId"); }
            if (sysUserGrpSId == null) { throw new ArgumentNullException("sysUserGrpSId"); }
            string[] exceptionSIds = this.SystemIdVerifier.CheckValid(actorSId, sysUserGrpSId);
            ...
            try
            {
                ...
                transSet.SmartAdd("SID", sysUserGrpSId.ToString(), GenericDBType.Char, false);
        ...
        /// </summary>
        /// <param name="actorSId">操作人系統代號。為客戶端在操作時的操作人系統代號。</param>
        /// <param name="sysUserGrpSId">系統使用者群組系統代號。</param>
        /// <param name="enabled">資料的啟用狀態。</param>
        /// <param name="name">名稱。</param>
        /// <param name="userRight">使用者權限（null 或 empty 則直接設為 DBNull）。</param>
        /// <param name="desc">說明（null 或 empty 則直接設為 DBNull）。</param>
        /// <param name="sort">排序。</param>
        /// <returns>EzCoding.Returner。</returns>
        /// <exception cref="System.ArgumentNullException">參數 actorSId 不允許為 null 值。</exception>
        /// <exception cref="System.ArgumentNullException">參數 sysUserGrpSId 不允許為 null 值。</exception>
        /// <exception cref="EzCoding.LibraryException">系統代號（ISystemId）的格式不正確。</exception>
        public Returner Modify(ISystemId actorSId, ISystemId sysUserGrpSId, bool enabled, string name, string userRight, string desc, int sort)
        {
            if (actorSId == null) { throw new ArgumentNullException("actorSId"); }
            if (sysUserGrpSId == null) { throw new ArgumentNullException("sysUserGrpSId"); }
            string[] exceptionSIds = this.SystemIdVerifier.CheckValid(actorSId, sysUserGrpSId);
            ...
            try
            {
                ...
                condsMain.Add(base.PrimaryKey, SqlCond.EqualTo, sysUserGrpSId.ToString(), GenericDBType.Char, string.Empty);
        ...
        /// <exception cref="System.ArgumentNullException">參數 sysUserGrpSId 不允許為 null 值。</exception>
        /// <exception cref="EzCoding.LibraryException">系統代號（ISystemId）的格式不正確。</exception>
        public Returner UpdateCustSort(ISystemId sysUserGrpSId, int val)
        {
            if (sysUserGrpSId == null) { throw new ArgumentNullException("sysUserGrpSId"); }
            string[] exceptionSIds = this.SystemIdVerifier.CheckValid(sysUserGrpSId);
            if (exceptionSIds.Length > 0)
            {
                throw new LibraryException(string.Format("invaild ISystemId ({0}) format.", string.Join("、", exceptionSIds)));
            }
        ...
        public class InfoConds
        {
            ...
            public InfoConds(ISystemId[] sysUserGrpSIds)
            {
        ...
        SqlCondsSet GetInfoCondsSet(InfoConds qConds)
        {
            ...
            custEntity.Append("SELECT [SYS_USER_GRP].* ");
            custEntity.Append("FROM [SYS_USER_GRP] ");

            var sqlConds = new List<string>();

            if (qConds.SysUserGrpSIds != null && qConds.SysUserGrpSIds.Length > 0)
            {
                sqlConds.Add(custEntity.BuildConds(string.Empty, "SID", SqlOperator.EqualTo, SystemId.ToString(qConds.SysUserGrpSIds), GenericDBType.Char));
            }
